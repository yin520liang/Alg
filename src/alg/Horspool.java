...
public class Horspool {
	...
	public static void main(String[] args) {
		...
		String s = null;
		try {
			s = FileUtils.readFileToString(new File("wiki-horspool.txt"));
		} catch (IOException e) {
	...
	/*
	 * void buildBloomFilter(char[] pch) { mask = 0; for (char c : pch) { mask
	 * |= 1 << (c & 0x1f); } }
	 * 
	 * boolean bloomContains(char c) { int tmp = 1 << (0x1f & c); return tmp ==
	 * (tmp & mask); }
	 */
}
